...
import { POKEMONCARD_IMAGES_QUEUE } from './discovery.ts';
...
/**
 * Attaches successfully downloaded pokemon-card.com images to their card as
 * a primary asset. Only touches cards TCGdex never supplied an image for
 * (`image_url IS NULL`): a later TCGdex materialize always wins if it ever
 * catches up, because `upsertCard`'s COALESCE never overwrites an existing
 * non-null image_url, and this function likewise never overwrites one.
 */
export function linkPokemonCardAssets(db: DatabaseSync, at: string): number {
  const rows = db
    .prepare(
      `SELECT o.scope_key, o.hash, ro.media_type, a.request_url
       FROM observations o
       ...
       WHERE w.source = 'pokemoncard' AND w.queue = ? AND o.entity_type = 'card_image'
         AND NOT EXISTS (
    ...
    .all(POKEMONCARD_IMAGES_QUEUE) as unknown as LinkableRow[];
  ...
  for (const row of rows) {
    ...
    ).run(cardId, row.hash, row.request_url, row.media_type, at, at);
